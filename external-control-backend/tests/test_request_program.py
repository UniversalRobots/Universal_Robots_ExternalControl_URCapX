...
def test_send_command_no_data(monkeypatch):
    ...
    assert (
        'Error while requesting code from 127.0.0.1:1234 -- Did not receive any script lines' in str(exc.value)
    )
    assert dummy.closed
...
def test_send_command_timeout(monkeypatch):
    ...
    with pytest.raises(Exception) as exc:
        rp.send_command('request_program\n')
    assert 'Timeout while receiving data from 127.0.0.1:1234' in str(exc.value)
    assert dummy.closed
